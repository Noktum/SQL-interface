...
class win2(Main):
    ...
    def start(self):
        ...
        # Поле 2
        text3 = tk.Label(window2, text="Введите строку")
    ...
    def to_sql(self):
        stroka = page2.entry_widget_4.get()
        ...
        first = stroka[:20]
        last = stroka[-15:]
        msg.showinfo("Ответ", str(vowel) + '\n' + str(consonant) + '\n' + first + '\n' + last)

        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql3 = f"INSERT INTO {page1.entry_widget_2.get()} (ID, Строка, Гласные, Согласные, Первые, Последние) VALUES (%s, %s, %s, %s, %s, %s)"
            cursor.execute(self.sql3, (page2.entry_widget_3.get(), stroka, vowel, consonant, first, last))
...
class BD(Main):
    ...
    def createtb(self):
        Main.connection1(self)
        with self.connection1.cursor() as cursor:
            self.sql2 = f"CREATE TABLE IF NOT EXISTS {page1.entry_widget_2.get()} (ID int, Строка varchar(200), Гласные int, Согласные int, Первые varchar(20), Последние varchar(15));"
